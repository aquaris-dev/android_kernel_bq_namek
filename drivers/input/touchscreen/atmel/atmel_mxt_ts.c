...
#define INFODEBUG	"leibo " // debug info ---leibo
...
static inline int board_hw_reset(const struct mxt_platform_data *pdata)
{
	//write your hw reset here, and return 0
	//if no, returen -EIO
...
#define EXAMPLE_FOR_KEY_CONTROLLED_EVENT
	...
	if (status & MXT_SCRAUX_STS_SUP) {
		...
		mxt_proc_gesture_messages(data, MXT_PROCI_TOUCHSUPPRESSION_T42,
			T42_KEY, NULL);
	}
...
static int mxt_read_info_block(struct mxt_data *data)
{
	...
	if ((data->info_crc == 0) || (data->info_crc != calculated_crc)) {
		...
		dev_err(&client->dev, "info block size %zu\n",size);
...
// end ----leibo 
	...
	{
		printk("leibo power on %s, power_ldo_gpio\n", __func__);
...
power_off:
	...
	if (gpio_is_valid(data->pdata->power_ldo_gpio))
	{
		printk("leibo power off ldo gpio!!!\n");
...
static void mxt_regulator_disable(struct mxt_data *data)
{
	...
	mxt_power_on(data, false);        //added by yong.bo on 2015/05/15 for tp power consumption
	mdelay(20);
}
...
//end ---leibo
#endif
	if (gpio_is_valid(data->pdata->power_ldo_gpio))// ldo_gpio may the GPIO_24, it used to enable 3V3 power --leibo
	{
		printk("leibo %s, power_ldo_gpio valid\n", __func__);
		...
		rc = gpio_direction_output(data->pdata->power_ldo_gpio, 1);// pull up, power 3v3.
		if (rc)
		{
		...
		printk("leibo  power_ldo_gpio unvalid!!!!\n");
#if 0
	else
	{
		printk("INFODEBUG%s, regulator\n", __func__);
...
#else
  mxt_power_init(data, true); // power on --leibo
#endif
	data->use_regulator = false; // double click mode no need power down.
	mxt_regulator_enable(data); //power sequence(power on & reset) --leibo
	...
	*/
	...
	if (prop) {
		...
		keymap[T15_T97_KEY][2] = KEY_MENU;
                keymap[T93_KEY][0] = KEY_WAKEUP; //for double tap wake up key
#endif
...
#if defined(CONFIG_MXT_PLUGIN_SUPPORT)
static void mxt_get_suffix_pid_name(struct device *dev)
{
	struct mxt_data *data = dev_get_drvdata(dev);
	char suffix[16];
	int ret; 
...
//Morven
static void update_keymap(struct mxt_platform_data *pdata)
{
	...
	pdata->keymap[T15_T97_KEY][2] = KEY_MENU;
        pdata->keymap[T93_KEY][0] = KEY_WAKEUP; //for double tap wake up key
#endif
	return;
}
#endif
...
#ifdef CONFIG_OF
//	dev_info(&client->dev, INFODEBUG"%s: parse device tree!\n",
//			__func__);
	if (!data->pdata && client->dev.of_node)
		data->pdata = mxt_parse_dt(client);
#endif
//		dev_info(&client->dev, INFODEBUG"%s: GPIO init!\n",
//				__func__);
        board_gpio_init(data->pdata); // set irq & reset gpio direction(input/output) --leibo
	if (!data->pdata) {
...
#endif
//	dev_info(&client->dev, INFODEBUG"%s: pinctrl init!\n",
//			__func__);

	 error = mxt_pinctrl_init(data); // ??? ---leibo
...
//	dev_info(&client->dev, INFODEBUG"%s: power on & reset!\n",
//			__func__);
...
/* add by yong.bo on 2015/09/08  add tp information for bug id bugzilla 99486 NAMEK-1780 */
	...
	error = class_create_file(touchscreen_class, &class_attr_ts_info);
	if (error < 0) {
...
/*end*/
	...
	if (!strncmp(data->plug.suffix_pid_name, ".03", 3) ||
		!strncmp(data->plug.suffix_pid_name, ".04", 3) ||
		!strncmp(data->plug.suffix_pid_name, ".05", 3) ||
		!strncmp(data->plug.suffix_pid_name, ".06", 3)) {
		if (data->pdata)
			update_keymap(data->pdata);
	}
#endif
#ifdef SUPPORT_READ_TP_VERSION
	memset(tp_version, 0, sizeof(tp_version));
	sprintf(tp_version, "Firmware version:0x%06X\n",data->config_crc);
	init_tp_fm_info(0,tp_version,"IC:ATMEL");
#endif
#if defined(CONFIG_FB_PM)
